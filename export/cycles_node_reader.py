...
def _node(node, output_socket, props, material, luxcore_name=None, obj_name="", group_node_stack=None):
    ...
    if node.bl_idname == "ShaderNodeBsdfPrincipled":
        ...
        # Check if the Mapping node is linked to a Texture Image node
        vector_output = utils_node.get_link(node.outputs["Vector"])
        if vector_output and vector_output.to_node.bl_idname == "ShaderNodeTexImage":
            tex_image_node = vector_output.to_node

            # Check if the Mapping node's input is linked to a TexCoord UV output
            vector_input = utils_node.get_link(node.inputs["Vector"])
            if vector_input and vector_input.from_node.bl_idname == "ShaderNodeTexCoord":
                tex_coord_node = vector_input.from_node

                # Extract scale values for X (uscale), Y (vscale), and Z (ignored for 2D mapping)
                uscale = node.inputs["Scale"].default_value[0]  # X-axis scale
                vscale = node.inputs["Scale"].default_value[1]  # Y-axis scale
                zscale = node.inputs["Scale"].default_value[2]  # Z-axis scale (unused for 2D)

                # Extract the offset (location) values
                udelta = node.inputs["Location"].default_value[0]  # X-axis offset
                vdelta = node.inputs["Location"].default_value[1]  # Y-axis offset

                # Define LuxCoreNodeTexMapping2D
                mapping_name = luxcore_name + "_mapping2d"
                mapping_prefix = f"scene.textures.{mapping_name}."
                mapping_definitions = {
                    "type": "mapping2d",
                    "udelta": udelta,  # Apply offset in X (translated to LuxCore)
                    "vdelta": vdelta,  # Apply offset in Y (translated to LuxCore)
                    "scale": [uscale, vscale],  # Apply independent scaling: X = uscale, Y = vscale
                    "use_uniform_scale": False,  # Explicitly disable uniform scaling
                }
                props.Set(utils.create_props(mapping_prefix, mapping_definitions))

                # Link the LuxCoreNodeTexMapping2D to LuxCoreNodeTexImagemap
                image_name = luxcore_name + "_imagemap"
                image_prefix = f"scene.textures.{image_name}."
                image_definitions = {
                    "type": "imagemap",
                    "mapping": mapping_name,
                    # Other parameters for the imagemap node
                }
                props.Set(utils.create_props(image_prefix, image_definitions))
                return image_name
